...
app.post('/api/login', async (req, res) => {
  try {
    ...
    const userAgent = req.headers['user-agent'] || 'Unknown';
    const isMobile = /Mobile|Android|iPhone|iPad/.test(userAgent);
    console.log(`🔐 Login attempt - Mobile: ${isMobile}, User: ${username}, UA: ${userAgent.substring(0, 50)}...`);
    ...
    db.get(
      ...
      async (err, user) => {
        ...
        res.json({
          ...
          debug: isMobile ? { mobile: true, cookieOptions } : undefined
        });
      }
    );
  } catch (error) {
...
app.get('/debug/auth-info', (req, res) => {
  ...
  if (cookies.auth_token) {
    try {
      tokenInfo = jwt.verify(cookies.auth_token, JWT_SECRET);
    } catch (error) {
  ...
  res.json({
    ...
    authTokenLength: cookies.auth_token ? cookies.auth_token.length : 0,
    tokenInfo: tokenInfo,
    userAgent: userAgent,
    isMobile: isMobile,
